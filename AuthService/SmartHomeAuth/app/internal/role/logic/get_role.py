...
async def get_role(name: str)->Role | None:
	try:
		return await Role.objects.select_related("privileges").get_or_none(role_name=name)
...
async def get_role_by_id(id: str)->Role | None:
	try:
		return await Role.objects.select_related("privileges").get_or_none(id=id)
	except Exception as e:
		logger.error(f"error get role: {e}")
		raise
...
async def get_role_all()->List[Role]:
	try:
		return await Role.objects.select_related("privileges").all()
	except Exception as e:
