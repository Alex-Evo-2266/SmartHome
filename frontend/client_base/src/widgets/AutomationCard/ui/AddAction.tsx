...
//     const [objectSecrch, setObjectSecrch] = useState(false)
//     const [fieldSecrch, setFieldSecrch] = useState(false)
...
//         if(service === "device")
//         {
//             const fieldCond = devicesData.find(item=>item.system_name === object)?.fields?.find(fieldItem=>fieldItem.name === field)
//             console.log(fieldCond)
//             if(!fieldCond)
//                 return {type: "none"}
//             else if(fieldCond.type === TypeDeviceField.ENUM)
//                 return {type: "select", items: fieldCond.enum_values?.split(",").map(item=>item.trim())}
//             else if(fieldCond.type === TypeDeviceField.BINARY)
//                 return {type: "select", items: [{title:"on", value: "1"}, {title: "off", value: "0"}, {title: "target", value: "target"}]}
//             else if(fieldCond.type === TypeDeviceField.NUMBER)
...
//     const valid = (service:string, object:string, field:string, data:string) =>{
//         if(service === "delay")
//             if(data !== "")
//                 return true
//         if(service === "device")
//             if(data !== "" && object !== "" && field !== "")
//                 return true
//         return false
//     }

//     const save = useCallback(()=>{
//         if(valid(service, object, field, data))
//         {
//             onSave({service, object, field, data, type_set: SetType.DATA})
//             setTimeout(onHide, 0)
//         }
//     },[service, object, data, field, onSave, onHide])
...
//                     <TextField placeholder="object" border readOnly value={object} onClick={()=>setObjectSecrch(true)}/>:
//                     null
//                 }
//                 {
//                     (FIELD_ENABLE.includes(service) && (object !== "" || !OBJECT_REQUIRED.includes(service)))?
//                     <TextField placeholder="field" border readOnly value={field} onClick={()=>setFieldSecrch(true)}/>:
//                     null
//                 }
//                 {
//                     dataConf && 
//                         (dataConf.type === "text"?
//                         <TextField placeholder="data" border value={data} onChange={(e)=>DataHeandler(e.target.value)}/>:
//                         dataConf.type === "number"?
//                         <NumberField placeholder="data" max={dataConf.max} min={dataConf.min} border value={Number(data)} onChange={(value)=>DataHeandler(String(value))}/>:
//                         dataConf.type === "select"?
//                         <SelectField placeholder="data" border items={dataConf.items ?? []} value={data} onChange={value=>DataHeandler(value)}/>:
//                         null)
//                 }
//             </div>
//         </BasicTemplateDialog>
...
//             objectSecrch &&
//             <DialogPortal>
//                 <SelectionDialog header="get object" items={getObject()} onHide={()=>setObjectSecrch(false)} onSuccess={objectHeandler}/>
//             </DialogPortal>
//         }
//         {
//             fieldSecrch &&
//             <DialogPortal>
//                 <SelectionDialog header="get data" items={getField()} onHide={()=>setFieldSecrch(false)} onSuccess={fieldHeandler}/>
//             </DialogPortal>
//         }
//         </>
