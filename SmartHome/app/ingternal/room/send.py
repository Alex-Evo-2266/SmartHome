...
#     snapshots = room_store.get_all_snapshots()
...
async def send_room_patch_data(patchs):
    try:
        await WebSocketMenager.send_information("send_room_patch", patchs.model_dump())
        logger.info("Device data successfully sent.")
    except Exception as e:
        logger.error(f"Error sending device data: {str(e)}")
...
async def send_room_snapshots_data(snapshots):
    try:
        ...
        await WebSocketMenager.send_information("send_room", arr)
        logger.info("Device data successfully sent.")
    except Exception as e:
        logger.error(f"Error sending device data: {str(e)}")
