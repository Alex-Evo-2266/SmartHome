...
from app.ingternal.device.arrays.DevicesArrey import DevicesArrey, DevicesArreyItem
from app.ingternal.device.arrays.DeviceClasses import DeviceClasses
from app.ingternal.device.schemas.enums import StatusDevice, DeviceStatusField, DeviceGetData
...
async def polling(device: Device):
	try:
		logger.debug("get_device function")
		device:BaseDevice
		if not device:
			raise DeviceNotFound()
		if device.type_get_data == DeviceGetData.PUSH:
			return
		if device.status == DeviceStatusField.UNLINK:
			return await get_default_data(device, StatusDevice.UNLINK)
		device_connect = DevicesArrey.get(device.system_name)
