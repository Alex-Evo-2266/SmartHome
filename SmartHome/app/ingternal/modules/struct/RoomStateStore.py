...
from app.ingternal.modules.struct.DeviceStatusStore import DevicePatch, DeviceSnapshot, DeviceStatusStore
...
async def room_state_init(state: RoomStateStore, device_store: DeviceStatusStore):

    try:
        for type_device in types:
            state.register_type(type_device)
    except Exception as e:
        print(e)

    device_store.subscribe_patch_global("room_state", state.on_device_patch)
    await device_store.subscribe_snapshot_global_async("room_state", state.on_device_snapshot)
